...
mkdir -p experiments
set folder experiments/q1-bigstep-$(date '+%Y-%m-%d-%H-%M-%S')
mkdir $folder
...
set benchfile benchmarks/bigstep.json
set epsilons 0.1 0.01 0.0001 1e-05
if test (count $argv) -gt 1
    ...
    set timeout 100
    set benchfile benchmarks/bigstep_short.json
    set epsilons 1e-05
end
...
function plot --description "plot <format> <separate-legend> <dpi> <title>"
    for epsilon in $epsilons
        python csv_to_scatter.py $result_file true false "bigstep" "no bigstep" --compare-by "Time (wall)" --max 4 --filter "Epsilon:$epsilon;Region Bound:0!0.1!1e-06!0,0.8!0.2,1" --output-pdf $folder/"time-$(string replace . , $epsilon).$argv[1]" --separate-legend $argv[2] --figsize $argv[3] --title $argv[4]
