...
module PM
    ...
    [tick0] state=0 & q*10 > T3*QMAX & q*10 <= T4*QMAX -> P41 : (pm'=0) + (1-P41) : (state'=1);
    [tick0] state=1 & q*10 > T3*QMAX & q*10 <= T4*QMAX -> P42 : (pm'=1) & (state'=0) + (1-P42) : (pm'=2) & (state'=0);

    [tick0] state=0 & q*10 > T4*QMAX & q*10 <= T5*QMAX -> P51 : (pm'=0) + (1-P51) : (state'=1);
    [tick0] state=1 & q*10 > T4*QMAX & q*10 <= T5*QMAX -> P52 : (pm'=1) & (state'=0) + (1-P52) : (pm'=2) & (state'=0);

    [tick0] state=0 & q*10 > T5*QMAX & q*10 <= T6*QMAX -> P61 : (pm'=0) + (1-P61) : (state'=1);
    [tick0] state=1 & q*10 > T5*QMAX & q*10 <= T6*QMAX -> P62 : (pm'=1) & (state'=0) + (1-P62) : (pm'=2) & (state'=0);

    [tick0] state=0 & q*10 > T6*QMAX & q*10 <= T7*QMAX -> P71 : (pm'=0) + (1-P71) : (state'=1);
    [tick0] state=1 & q*10 > T6*QMAX & q*10 <= T7*QMAX -> P72 : (pm'=1) & (state'=0) + (1-P72) : (pm'=2) & (state'=0);

    [tick0] state=0 & q*10 > T7*QMAX -> P81 : (pm'=0) + (1-P81) : (state'=1);
    [tick0] state=1 & q*10 > T7*QMAX -> P82 : (pm'=1) & (state'=0) + (1-P82) : (pm'=2) & (state'=0);
endmodule
...
module BAT
    ...
    batstate : [0..1] init 0; // internal state
    [tick1] bat=0 ->true;
    [tick1] bat=1 & batstate=0 -> 1 : (batstate'=1);
    [] bat=1 & batstate=1 -> empty : (bat'=0) & (batstate'=0) + 1-empty : (batstate'=0);
endmodule
