...
parser.add_argument('--storm-pars', type=str, default="~/git/storm/build/bin/storm-pars",
...
parser.add_argument('--storm-pomdp', type=str, default="~/git/storm/build/bin/storm-pomdp",
...
for config_file in glob.glob(args.folder + "/**/config.json"):
    ...
    for invocation in config:
        ...
        if "pomdp" in invocation:
            drn_file = getcwd() + "/.build/" + folder + "/" + "_".join([invocation["pomdp"].split(".")[0], prop_dict["type"], prop_dict["dir"], str(prop_dict["bound"]), prop_dict["label"].replace(" ", "_").replace("\"", ""), prop_dict["reward_model"] if "reward_model" in prop_dict else "", constant_string, str(invocation["memory_bound"])]) + ".drn"
            storm_pomdp_command = "{binary} --parametric-drn {drn_file} --prism {pomdp} -prop \"{prop}\" -const '{constant_string}' " + \
        ...
        for i in range(args.times):
            ...
            use_robust_pla = horizon > 1
            command = "time {binary} {file} {constants} --prop \"{property}\" {method} --mode partitioning --regionbound 0.01 --terminationCondition 0.01 {robust_pla} {exact} -bisim {additional_storm_args}".format(
                binary = args.storm_pars,
                file = "--explicit-drn " + drn_file if drn_file else "--prism " + prism_file,
                constants = "" if drn_file else "-const " + constant_string,
                property = escaped_prop,
                method = f"--big-step {horizon} {timetravel}" if horizon >= 1 else "",
                robust_pla = "--regionverif:engine robustpl --minmax:method vi" if use_robust_pla else "",
            ...
            manual_script += "( echo \"" + json_str.replace("\"", "\\\"") + "\"" + " && timeout 300 " + command + " ) >> " + "output/" + json_str.replace("\"", "").replace("{", "").replace("}", "").replace(".", "").replace(" ", "").replace(",", "-").replace(">", "greater").replace("<", "less") + ".out 2>&1" + "\n"
...
slurm_command_file_name = "scripts/slurm_commands" + str(int(datetime.now().timestamp())) + ".sh"
manual_command_file_name = "scripts/manual_commands" + str(int(datetime.now().timestamp())) + ".sh"
...
for template, command_file in {"hpc": slurm_command_file_name, "parallel": manual_command_file_name}.items():
